    ...
    <Content :style="{ padding: '24px', minHeight: '280px', width: '100%' }">
      ...
      <Collapse value="1"  simple>
        ...
        <FormItem label="日记标签">
         ...
         <FormItem >
           <Tag type="border"   closable  class="tagg"  :color="tagsColor[index]" v-for="(item,index) of diary.tags" :key="index" @on-close="handleClose" :name="item" >{{item}}</Tag>
        </FormItem>
    </Form>
<!--          这一行</p>不能删-->
          </p>
  ...
  methods: {
    ...
    tags(value) {
      ...
      if (this.diary.tags.length < 10 && value !== "") {
        ...
        this.$Message.warning("日记标签不能为空和多余10个");
      }
    },
    //标签关闭
