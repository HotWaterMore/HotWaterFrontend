        ...
        >
          <template slot-scope="{ row, index }" slot="action">
            ...
            <Button
              ...
              type="error"
              size="small"
              @click="(modal = true), (diaryId = row.diaryId)"
              >删除</Button
            >
          </template>
        </Table>
...
export default {
  ...
  methods: {
    //查看日记
    diartDetails(row) {
      this.$router.push({ path: `/diaryDetails/${row.diaryId}` });
    },
    pageChange(pno) {
   ...
   //修改日记
    updateDiary(diaryId) {
       this.$router.push({path:`/updateDiary/${diaryId}`});
    },
    //删除日记
    remove() {
      ...
      setTimeout(() => {
        ...
        this.modal = false;
        this.request
          .httpPost(this.requestUrl.delDiary, {
    ...
    selectDiary() {
      this.request
        ...
        .then(res => {
          if (res.code === 200) {
            this.loading = false;
            this.data = res.data.diaries;
            this.diaryCount = res.data.count;
          } else {
            this.$Message.error("查询用户的所有日记失败");
          }
        });
    },
    //根据时间查询日记
  ...
  created() {
    ...
    if (kwd) {
      this.request
        .httpGet(this.requestUrl.selectTagDiary, {
          kwd: kwd,
          userId: this.diaryQuery.userId
        })
        .then(res => {
