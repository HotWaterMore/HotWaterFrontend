  ...
  <Layout :style="{ padding: '0 24px 24px' }">
    <Content :style="{ padding: '24px', minHeight: '280px', width: '100%' }">
      ...
      <Collapse value="1"  simple>
        ...
        <FormItem label="日记标题" >
            <Input prefix="ios-paper" type="text" :placeholder="diary.title" clearable v-model="diary.title">
            </Input>
        </FormItem>
        <FormItem label="隐私设置">
        ...
        <FormItem label="日记标签">
         ...
         <FormItem >
           <Tag type="border" closable class="tag" :color="tagsColor[index]" v-for="(item,index) of diary.tags" :key="index" @on-close="handleClose" :name="item" >{{item}}</Tag>
        </FormItem>
    </Form>
          </p>
        </Panel>
...
export default {
  ...
  data() {
    return {
      ...
      diary: {
        content: "", //富文本内容
        title: new Date().toLocaleDateString() + "日记", //日记标题
        tags:[], //日记标签
        privacy: "0",
        userId: JSON.parse(localStorage.getItem("userInfo")).userId,
        diaryId:''
  ...
  methods: {
    ...
    updateDiary() {
      if (this.diary.content.trim()) {
        this.request
          .httpPost(this.requestUrl.updateDiary, this.diary)
          .then(res => {
            if (res.code === 200) {
    ...
    tags(value) {
      this.tag = "";
      if (this.diary.tags.length < 10 && value !== "") {
        this.diary.tags.push(value);
      } else {
        this.$Message.warning("日记标签不能为空和多于10个");
      }
    },
    //标签关闭
    handleClose(event, name) {
      const index = this.diary.tags.indexOf(name);
      this.diary.tags.splice(index, 1);
    //   if(this.diary.tags.length==0){
    //        this.diary.tags.push(null);
    //   }
    },
     //返回上一页
    peve() {
      ...
      this.request
        ...
        .then(res => {
          if (res.code === 200) {
            ...
            this.diary.content = res.data.content;
            this.diary.tags = res.data.tags;
          } else {
            this.$Message.error("根据日记id查询日记失败")
          }
        });
    },
  },
